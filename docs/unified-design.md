...
- **`danso update check`** — 소비 쪽의 읽기 절반. 구현은 `danso-ops`가 아니라
  ...
  - **서명이 경계이고 전송은 아니다.** 연결을 쥔 쪽이 무엇을 주든
    ...
    그리고 **자체 벽시계 마감**. reqwest blocking의 `timeout`은 read마다
    재무장되어 *정체*만 묶고 전송은 묶지 않는다 — 실측: 5초마다 1바이트를
    보내는 소스가 30초 예산에서 150초까지 읽히고 있었다. 오류는 이유만 말하고
    **URL을 찍지 않는다**(릴리스 소스는 운영자 설정이다). 소스에 쿼리·프래그먼트는
    **금지**다 — 하위 경로를 뒤에 잇는 구조라 `…/rel?token=X`가
    `…/rel?token=X/SHA256SUMS`가 되고, 실패가 엉뚱하게 서명 오류로 나온다.
  - **"모른다"는 exit 0**이지 10이 아니다. `update rollback`이 아카이브 필드
    없는 레코드를 쓰므로, 10에 반응해 재설치하는 래퍼는 운영자가 방금 되돌린
    릴리스를 즉시 다시 깐다. **같은 타깃 아티팩트가 둘이면 exit 2** — 정렬된
    맵의 "첫 번째"는 사전순이고, `…-0.1.0-…`과 `…-0.2.0-…`을 함께 담은 릴리스가
    전자에서 "최신"으로 읽힌다.
